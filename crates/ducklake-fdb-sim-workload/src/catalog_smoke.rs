...
use ducklake_fdb_sim_model::{
    CatalogSmokeScenario, require_exactly_one_committed_file, require_no_retry_publication,
};
...
pub(crate) struct CatalogSmokeWorkload {
    name: String,
    profile: String,
    client_id: i32,
    client_count: i32,
...
impl CatalogSmokeWorkload {
    ...
    fn trace_step(&self, step: &'static str) {
        self.context.trace(
            Severity::Info,
            "DuckLakeFdbSimCatalogSmokeStep",
            details![
                "Layer" => "aux-ducklake",
                "Workload" => &self.name,
    ...
    fn trace_error(&mut self, step: &'static str, error: impl Into<String>) {
        ...
        self.context.trace(
            Severity::Error,
            "DuckLakeFdbSimCatalogSmokeError",
            details![
                "Layer" => "aux-ducklake",
                "Workload" => &self.name,
                "Profile" => &self.profile,
    ...
    async fn run_catalog_smoke(&mut self, db: SimDatabase) -> Result<(), String> {
        let scenario = CatalogSmokeScenario::for_client(&self.profile, self.client_id);
        let catalog = CatalogId(scenario.catalog_id);
        let table = TableId(scenario.table_id);
        let attempt = CommitAttemptId(scenario.first_attempt_id);
        ...
        let committed = kv
            ...
            .await
            .map_err(|err| format!("commit mutation: {err}"))?;
        ...
        if file.validity.begin_order.kind() != CatalogOrderKind::FdbVersionstamp {
            return Err("data file begin order is not an FDB versionstamp".to_owned());
        }
        self.commit_count += 1;
        ...
        let retry = kv
            ...
            .await
            .map_err(|err| format!("retry mutation: {err}"))?;
        require_no_retry_publication(&retry.data_files)?;
        self.recovery_count += 1;

        let attempt_row = kv
            .load_commit_attempt_async(catalog, Some(attempt))
            .await
            .map_err(|err| format!("load commit attempt: {err}"))?
            .ok_or_else(|| "commit attempt row missing after commit".to_owned())?;
        if attempt_row.commit_order != file.validity.begin_order {
            return Err("commit attempt order does not match data file order".to_owned());
        }

        let files = kv
            .list_current_data_files_async(catalog, table)
            .await
            .map_err(|err| format!("list current files: {err}"))?;
        if files != vec![file] {
            return Err(format!(
                "expected {} current data file, got {}",
                scenario.expected_current_file_count(),
                files.len()
            ));
        }
        Ok(())
...
impl RustWorkload for CatalogSmokeWorkload {
    ...
    async fn start(&mut self, db: SimDatabase) {
        ...
        if let Err(err) = self.run_catalog_smoke(db).await {
    ...
    fn get_metrics(&self, mut out: Metrics) {
        out.extend([
            metric("ducklake_catalog_smoke_setup_count", self.setup_count),
            metric("ducklake_catalog_smoke_start_count", self.start_count),
            metric("ducklake_catalog_smoke_check_count", self.check_count),
            metric("ducklake_catalog_smoke_commit_count", self.commit_count),
            metric("ducklake_catalog_smoke_recovery_count", self.recovery_count),
            metric("ducklake_catalog_smoke_error_count", self.error_count),
        ]);
    }
