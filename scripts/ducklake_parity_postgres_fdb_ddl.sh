...
SCENARIO_SQL="$ROOT_DIR/docs/parity/ducklake-fdb/sql/ddl_parity.sql"
READBACK_SQL="$ROOT_DIR/docs/parity/ducklake-fdb/sql/ddl_parity_readback.sql"
EXPECTED="$ROOT_DIR/docs/parity/ducklake-fdb/expected/ddl_parity.out"

fail() {
    echo "ducklake postgres/fdb ddl parity failure: $*" >&2
    exit 1
}
...
run_postgres_ddl() {
    ...
    metadata_schema="ducklake_ddl_parity_$(date +%s)_$$"
...
"
    [[ "$duckdb_status" -eq 0 ]] || {
        printf '%s\n' "$duckdb_output" >&2
        fail "Postgres-backed DDL parity failed"
    }
...
"
    [[ "$duckdb_status" -eq 0 ]] || {
        printf '%s\n' "$duckdb_output" >&2
        fail "FoundationDB-backed DDL parity failed"
    }
    assert_expected_output "$duckdb_output"
    printf '%s\n' "$duckdb_output"
}
...
echo "ddl_parity_step=postgres_ddl"
run_postgres_ddl "$tmp_root/postgres"
fdb_prefix="aux-ducklake-e2e/ddl_parity-ddl/$(date +%s)/$$/"
echo "ddl_parity_step=fdb_ddl"
run_fdb_ddl "$tmp_root/fdb" "$fdb_prefix"
echo "ducklake_parity_postgres_fdb_ddl=ok"
